...
- ✅ **流式审核**：实时显示审核进度和推理过程，支持渐进式结果展示
- ✅ **并行处理**：多张图片并行审核，性能提升 60-70%
- ✅ **RAG增强**：向量检索相关监管条文，提升准确率
- ✅ **完整知识库**：包含3篇核心监管文档，共134条监管条文
- ✅ **中英双语**：支持中英文界面切换，语言偏好自动保存
...
1. **语言切换**（可选）：点击右上角「中文 / EN」按钮切换界面语言
2. **注册**：首次使用请先注册账号
3. **登录**：使用用户名和密码登录
4. **配置 API 密钥**：进入「个人中心」→ 输入百炼 API 密钥 → 保存并验证
5. **审核**：返回首页，输入营销内容进行合规审核
...
<table>
<tr>
<td width="33%"><b>登录页面</b><br/><img src="docs/screenshots/01-login.png" width="100%"/></td>
<td width="33%"><b>注册页面</b><br/><img src="docs/screenshots/02-register.png" width="100%"/></td>
<td width="33%"><b>审核页面</b><br/><img src="docs/screenshots/03-review-empty.png" width="100%"/></td>
</tr>
<tr>
<td width="33%"><b>文本审核结果</b><br/><img src="docs/screenshots/04-review-text.png" width="100%"/></td>
<td width="33%"><b>图片审核结果</b><br/><img src="docs/screenshots/05-review-images.png" width="100%"/></td>
<td width="33%"><b>个人中心</b><br/><img src="docs/screenshots/06-profile.png" width="100%"/></td>
</tr>
</table>
...
### 前端界面
- 点击右上角「中文 / EN」按钮即可切换语言
- 语言偏好自动保存在浏览器 localStorage
- 所有用户可见文本均已翻译（登录、注册、审核、个人中心等）

### 后端 API
- 根据请求头 `Accept-Language` 返回对应语言的错误消息
- 支持中文（zh）和英文（en）
...
- [流式审核与性能优化](docs/STREAMING_AND_PERFORMANCE.md) - 流式审核功能和性能优化详解
